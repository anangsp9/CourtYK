    ...
    <div class="max-w-4xl mx-auto py-8 px-4">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-100 mb-6">
            ...
            <p class="text-lg text-gray-700">
                Harga: <span class="text-blue-600 font-semibold">Rp {{ number_format($court->price_per_hour, 0, ',', '.') }}</span> / jam
            </p>
        </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    @foreach($slots as $slot)
                        @if(in_array($slot, $bookedSlots))
                            <div class="bg-red-100 border border-red-200 p-3 rounded-lg text-center font-medium text-red-700 opacity-60 cursor-not-allowed">
                                {{ $slot }}
                                <br>
                                <span class="text-xs font-bold uppercase tracking-wider">Booked</span>
                            </div>
                        @else
                            <div class="bg-green-100 border border-green-200 p-3 rounded-lg text-center font-medium text-green-700">
                                {{ $slot }}
                                <br>
            ...
            <form action="{{ route('bookings.store', $court) }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="start_time" class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm px-3 py-2">
                    ...
                    <div>
                        ...
                        <select name="duration" class="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm px-3 py-2">
                ...
                <div class="pt-2">
                    <button type="submit" class="w-full sm:w-auto px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md shadow-sm transition-colors duration-150">
                        Konfirmasi Booking
                    </button>
                </div>
            </form>
