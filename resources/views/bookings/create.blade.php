...
<body class="bg-background text-on-background min-h-screen font-body-lg selection:bg-primary-fixed selection:text-on-primary-fixed">
    ...
    <header class="fixed top-0 w-full z-50 bg-surface/60 backdrop-blur-3xl border-b border-white/10 shadow-[0px_20px_40px_rgba(0,0,0,0.5)]">
        <div class="flex items-center justify-between px-gutter h-16 w-full max-w-7xl mx-auto">
            <a href="{{ route('dashboard') }}" class="font-display-lg text-headline-lg-mobile italic tracking-tighter text-primary-fixed">CourtYK</a>
            <div class="hidden md:flex gap-8 items-center">
                ...
                <a href="{{ route('bookings.index') }}"
                    class="text-on-surface-variant hover:text-primary-fixed font-label-sm uppercase tracking-widest transition-colors">My Bookings</a>
            </div>
            <div class="flex gap-4 items-center">
                <span class="hidden md:block text-on-surface-variant text-sm">{{ Auth::user()->name }}</span>
                <a href="{{ route('profile.edit') }}" class="material-symbols-outlined p-2 rounded-full hover:bg-white/5 transition-colors text-on-surface" title="Profile">person</a>
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button class="material-symbols-outlined p-2 rounded-full hover:bg-white/5 transition-colors text-on-surface" title="Logout">logout</button>
                </form>
            </div>
        </div>
    </header>
    ...
    <main class="pt-24 pb-32 px-gutter max-w-7xl mx-auto">
        ...
        <section class="mb-layer-gap relative overflow-hidden rounded-lg">
            <div class="absolute inset-0 z-0">
                ...
                @else
                    <div class="w-full h-full bg-gradient-to-br from-surface-container-highest to-background opacity-60"></div>
                @endif
                <div class="absolute inset-0 bg-gradient-to-t from-background via-background/40 to-transparent"></div>
            </div>
            <div class="relative z-10 p-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <span class="inline-block px-3 py-1 rounded-full bg-primary-fixed/20 text-primary-fixed font-label-sm uppercase tracking-widest mb-4 border border-primary-fixed/30">Premium Court</span>
                ...
                <div class="glass-panel p-6 rounded-lg text-left md:text-right min-w-[200px]">
                    ...
                    <div class="text-primary-fixed font-display-lg text-headline-lg-mobile neon-text-glow">Rp {{ number_format($court->price_per_hour, 0, ',', '.') }}<span class="text-title-md opacity-60">/hr</span></div>
                </div>
            </div>
        </section>
                ...
                <div class="glass-panel p-8 rounded-lg">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
                        <h3 class="font-title-md text-title-md text-primary flex items-center gap-2">
                            ...
                            Slot Waktu
                            <span class="text-label-sm text-on-surface-variant font-normal">({{ \Carbon\Carbon::parse($date)->format('d M Y') }})</span>
                        </h3>
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                        @foreach ($slots as $slot)
                            @if (in_array($slot, $bookedSlots))
                                <div class="flex flex-col items-center justify-center p-4 rounded-lg bg-surface-container-highest/40 border border-white/5 text-on-surface-variant opacity-40 cursor-not-allowed">
            ...
            <div class="lg:col-span-4 sticky top-24">
                <div class="glass-panel p-8 rounded-lg space-y-6">
                    ...
                    <form action="{{ route('bookings.store', $court) }}" method="POST" id="booking-form">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-label-sm text-on-surface-variant uppercase tracking-widest mb-2">Jam Mulai</label>
                                <select name="start_time_select" id="start_time_select"
                                    ...
                                    @foreach ($slots as $slot)
                                        @if (!in_array($slot, $bookedSlots))
                                            <option class="bg-surface" value="{{ $slot }}">{{ $slot }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-label-sm text-on-surface-variant uppercase tracking-widest mb-2">Durasi Main</label>
                        ...
                        <div class="space-y-3 mt-6">
                            ...
                            <div class="flex justify-between text-headline-lg-mobile font-headline-lg-mobile pt-2">
                                <span>Total</span>
                                <span class="text-primary-fixed neon-text-glow" id="total-display">Rp {{ number_format($court->price_per_hour, 0, ',', '.') }}</span>
                            </div>
                        </div>

                        <button type="submit" class="w-full py-4 mt-6 bg-primary-fixed text-on-primary-fixed font-title-md text-title-md rounded-lg electric-glow hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                            Konfirmasi Booking
    ...
    <nav class="md:hidden fixed bottom-0 w-full z-50 rounded-t-xl bg-surface-container-lowest/40 backdrop-blur-3xl border-t border-white/15 shadow-[0px_-10px_30px_rgba(0,0,0,0.3)]">
